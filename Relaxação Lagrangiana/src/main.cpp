...
vector<vector<double>> altera_matriz(Data data, vector<double> lambda)
{
    ...
    for (int i = 0; i < data.getDimension(); i++)
    {
        ...
        {
            data.distMatrix[i][j] = data.distMatrix[i][j] - lambda[i + 1] - lambda[j + 1];
        }
    }
}
...
Tree Subgradient(Data &data, double UB)
{
    ...
    Original_tree = Make1_Tree(data, lambda);
